...
import sys
sys.path.insert(9, 
    r'd:/abaqus_2021_tmp/Abaqus MainWindow GUI/abaqus temp/abaqus_plugins/husanjiaov22')
import hujiaokongv22_fun

def k1_huzg(partname_k, d_k, bate_k, Dh_k, yanshen_k, gunfeng_k):
    hujiaokongv22_fun.husanjiaov22(partname_k, d_k, bate_k, Dh_k, yanshen_k, gunfeng_k)
    ...
    a.Instance(name='hzg-1', part=p, dependent=ON)
    ...
    a = mdb.models['Model-1'].rootAssembly
    a.rotate(instanceList=('hzg-1', ), axisPoint=(-Rh_k, 0.0, 0.0), axisDirection=(
    ...
    a = mdb.models['Model-1'].rootAssembly
    a.translate(instanceList=('hzg-1', ), vector=(Rh_k, Rh_k, 0.0))
    a = mdb.models['Model-1'].rootAssembly
    p = mdb.models['Model-1'].parts[partname_k]
    a.Instance(name='hzg-2', part=p, dependent=ON)
    session.viewports['Viewport: 1'].view.setValues(width=246.697, height=110.179, 
        viewOffsetX=-0.169607, viewOffsetY=-1.16321)
    a = mdb.models['Model-1'].rootAssembly
    a.rotate(instanceList=('hzg-2', ), axisPoint=(-Rh_k, 0.0, 0.0), axisDirection=(
        0.0, 0.0, -10.0), angle=-30.0)
    a = mdb.models['Model-1'].rootAssembly
    a.translate(instanceList=('hzg-2', ), vector=(Rh_k-Rh_k*(math.sin(60*du)), -Rh_k/2, 0.0))
    a = mdb.models['Model-1'].rootAssembly
    p = mdb.models['Model-1'].parts[partname_k]
    a.Instance(name='hzg-3', part=p, dependent=ON)
    a = mdb.models['Model-1'].rootAssembly
    a.rotate(instanceList=('hzg-3', ), axisPoint=(-Rh_k, 0.0, 0.0), axisDirection=(
        0.0, 0.0, -10.0), angle=30.0)
    session.viewports['Viewport: 1'].view.setValues(width=373.542, height=166.831, 
        viewOffsetX=-6.4744, viewOffsetY=-1.00549)
    a = mdb.models['Model-1'].rootAssembly
    a.translate(instanceList=('hzg-3', ), vector=(Rh_k+Rh_k*(math.sin(60*du)), -Rh_k/2, 0.0))
    session.viewports['Viewport: 1'].view.setValues(session.views['Front'])
